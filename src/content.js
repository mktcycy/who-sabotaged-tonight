...
const EVENTS = [
  ...
  },
  {
    ...
    options: [
      ...
      option('B', '加發津貼繼續拚', '疲勞被換算成現金。', { money: -14, morale: 8, risk: 13 }),
      option('C', '舉辦抗壓講座', '要求過勞的人準時參加兩小時課程。', { money: -5, morale: -20, risk: 18 }, cond('morale', '<', 35, { risk: 12 }, '若事件開始時 Morale < 35，Risk 額外 +12。')),
    ],
  },
  {
    ...
    options: [
      ...
      option('C', '取消並寄感謝信', '感謝很滿，餐桌很空。', { money: 8, morale: -18, risk: 12 }),
    ],
  },
  {
    ...
    options: [
      option('A', '全面進軍', '可能成為傳奇，也可能成為教材。', { money: -20, morale: 10, risk: 24 }),
      option('B', '區域試點', '慢一點，但地圖還看得懂。', { money: -11, morale: 4, risk: 9 }),
  ...
  },
  {
    ...
    options: [
      ...
      option('B', '改找低價供應商', '規格看起來差不多，字體也差不多。', { money: 9, morale: -5, risk: 22 }),
      option('C', '自行生產', '昂貴轉型，但命運握在自己手上。', { money: -16, morale: 12, risk: 8 }),
    ],
  },
  {
    ...
    options: [
      ...
      option('B', '法律強硬回應', '讓律師和記者一起加班。', { money: -14, morale: -8, risk: 17 }),
      option('C', '製造新話題轉移', '用另一場火蓋住這場火。', { money: -9, morale: -6, risk: 22 }, cond('risk', '>=', 65, { risk: 8 }, '若事件開始時 Risk ≥ 65，Risk 額外 +8。')),
    ],
  },
  {
    ...
    options: [
      option('A', '全力接單', '資金大增，所有警報一起亮。', { money: 28, morale: -18, risk: 25 }),
      option('B', '縮小規模承接', '少賺一點，少失眠一點。', { money: 16, morale: -7, risk: 12 }),
...
const MISSIONS = [
  { id: 'S01', text: '最終 Money ≥ 50', type: 'STAT', stat: 'money', operator: '>=', value: 50 },
  { id: 'S02', text: '最終 Money ≥ 65', type: 'STAT', stat: 'money', operator: '>=', value: 65 },
  { id: 'S03', text: '最終 Money ≥ 75', type: 'STAT', stat: 'money', operator: '>=', value: 75 },
  ...
  { id: 'S06', text: '最終 Morale ≥ 75', type: 'STAT', stat: 'morale', operator: '>=', value: 75 },
  { id: 'S07', text: '最終 Risk ≤ 35', type: 'STAT', stat: 'risk', operator: '<=', value: 35 },
  { id: 'S08', text: '最終 Risk ≤ 45', type: 'STAT', stat: 'risk', operator: '<=', value: 45 },
  { id: 'S09', text: '最終 Risk 介於 50–70（含）', type: 'RANGE', stat: 'risk', min: 50, max: 70 },
  { id: 'S10', text: '最終 Money 介於 35–60（含）', type: 'RANGE', stat: 'money', min: 35, max: 60 },
  { id: 'D01', text: '最終 Money ≥ 40 且 Morale ≥ 50', type: 'ALL_STATS', rules: [{ stat: 'money', operator: '>=', value: 40 }, { stat: 'morale', operator: '>=', value: 50 }] },
  { id: 'D02', text: '最終 Money ≥ 50 且 Risk ≤ 60', type: 'ALL_STATS', rules: [{ stat: 'money', operator: '>=', value: 50 }, { stat: 'risk', operator: '<=', value: 60 }] },
  { id: 'D03', text: '最終 Morale ≥ 55 且 Risk ≤ 55', type: 'ALL_STATS', rules: [{ stat: 'morale', operator: '>=', value: 55 }, { stat: 'risk', operator: '<=', value: 55 }] },
  { id: 'D04', text: '最終 Money ≥ 55 且 Morale ≥ 45', type: 'ALL_STATS', rules: [{ stat: 'money', operator: '>=', value: 55 }, { stat: 'morale', operator: '>=', value: 45 }] },
  { id: 'D05', text: '最終三項數值皆在 25–80（含）', type: 'ALL_RANGE', min: 25, max: 80 },
  { id: 'D06', text: '最終 Money + Morale ≥ 110', type: 'SUM', stats: ['money', 'morale'], operator: '>=', value: 110 },
  { id: 'B01', text: '最後三回合至少兩次投給最終執行方案', type: 'FINAL_CHOICE', rounds: [6, 7, 8], count: 2 },
  { id: 'B02', text: '全場至少三次投給該回合少數選項', type: 'MINORITY', count: 3 },
  { id: 'B03', text: '全場至少五次投給最終執行方案', type: 'FINAL_CHOICE', rounds: [1, 2, 3, 4, 5, 6, 7, 8], count: 5 },
