...
window.App = (() => {
  const PHASES = {
    ...
    INTEL: '私人情報', DISCUSSION: '討論時間', VOTE: '匿名投票', RESULT: '公布結果',
  ...
  function eventHtml(event, vote = false, selected = null, disabled = false) {
    if (!event) return '';
    return `<div class="eyebrow">${escapeHtml(event.stage)} EVENT</div><h2 class="title">${escapeHtml(event.title)}</h2>
      <p class="subtitle">${escapeHtml(event.description)}</p><div class="options">${event.options.map((option) => {
        ...
        return `<${tag} class="option ${vote ? 'vote' : ''} ${selected === option.id ? 'selected' : ''}" ${vote ? `data-option="${option.id}" ${disabled ? 'disabled' : ''}` : ''}>
